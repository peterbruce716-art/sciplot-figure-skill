...
- Treat `baseline_visibility: occluded_by_front_groups` as an explicit calibration fact for a covered background or middle bar. Accept a non-baseline-connected run only when overlapping foreground geometry is present and either a narrow same-color edge continues to the baseline or the foreground fill bridges immediately below the candidate run. Otherwise omit the row and fail the audit; compact legend swatches must not become bar values.
