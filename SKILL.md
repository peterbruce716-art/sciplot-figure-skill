...
- Use `scripts/digitize_grouped_bar_raster.py` for raster-only grouped bars after explicitly calibrating each panel rectangle, y-axis mapping, category centers, group offsets, widths, and fill colors. For scaffolded configs, require `calibration_status: pass` and an empty `unresolved_segments` list. Keep its CSV and audit JSON as companion evidence; the extracted values remain `digitized_raster`, not raw data.
